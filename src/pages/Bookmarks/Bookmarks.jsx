...
const Bookmarks = () => {
    ...
    const [selectedTopic, setSelectedTopic] = useState("all");
      const [error, setError] = useState(null); // Thêm dòng này
  ...
  const fetchBookmarks = async () => {
    try {
      setLoading(true);
      const { data } = await postsService.getBookMarkPost();
  ...
  const handleBookmark = async(postId) => {
    ...
    // Lấy tất cả chủ đề duy nhất từ bookmark
    const availableTopics = [
        ...new Map(
            bookmarks
                .flatMap((b) => Array.isArray(b.topics) ? b.topics : [])
    ...
    const filteredBookmarks = bookmarks.filter((bookmark) => {
        const matchesSearch =
            ...
            bookmark.user?.first_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            bookmark.user?.last_name?.toLowerCase().includes(searchTerm.toLowerCase());

        const matchesTopic =
            selectedTopic === "all" ||
            (Array.isArray(bookmark.topics) &&
                bookmark.topics.some((t) => t.name === selectedTopic));
                            ...
                            {filteredBookmarks.map((bookmark) => (
                                <div key={bookmark.id} className={styles.bookmarkItem}>
                                    <PostCard
                                        ...
                                        excerpt={bookmark.meta_description || bookmark.excerpt}
