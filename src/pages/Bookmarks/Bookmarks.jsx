...
const Bookmarks = () => {
  ...
  try {
    ...
    // Cập nhật lại state bookmarks ngay trong client
    setBookmarks((prev) =>
      prev.map((b) =>
        b.id === postId ? { ...b, is_like: !b.is_like, likesCount: b.is_like ? b.likesCount - 1 : b.likesCount + 1 } : b
      )
    );
  } catch (error) {
    console.log(error);
  ...
  const filteredBookmarks = bookmarks.filter((bookmark) => {
    const matchesSearch =
      ...
      bookmark.user?.first_name
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      bookmark.user?.last_name
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase());
